...
const ThongTinphim = ({ maLichChieu }) => {
  ...
  const { listGheDangDat, sum, listGhe } = useSelector(
      ...
      <div className="container h-screen w-2/5">
        <div className="flex justify-center items-center h-full">
          <div className="rounded-xl border-2 text-white w-4/5">
            ...
            <div className="rounded border text-gray-900">
              Tên phim :
              <span className="text-white-800 font-bold text-xl">
                {" "}
                {dataPhim?.tenPhim}
              </span>
            </div>
            <div className="rounded border  text-gray-900">
              Giờ Chiếu:{" "}
              <span className="text-white-800 font-bold text-sm">
                {" "}
                {dataPhim?.gioChieu}
              </span>
            </div>
            ...
            <div className="rounded border">
              ...
              <div className=" text-gray-900" onClick={openModal}>
                Tổng Tiền: {renderSum()}{" "}
                <button
                  className={` ml-10 border-2  rounded-xl h-8 ${
                    isDisabled
      ...
      {isModalOpen && (
        <div class="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50">
          <div class="bg-white p-8 rounded-lg">
            <div className="text-center text-xl font-bold text-red-500 mb-5">
              Xác Nhận Thanh Toán
            </div>
            <div>
              Số ghế đã chọn <span>{renderListGhe()}</span>
            </div>
            <div>
              Tổng số tiền thanh toán <span>{renderSum()}</span>{" "}
            </div>
